<?php
add_action( 'admin_enqueue_scripts', 'cptslotsb_feedback_insert_adminScripts', 1); 
add_action( 'wp_ajax_cptslotsb_feedback', 'cptslotsb_feedback' );
$cptslotsb_plugfile =  'wp-time-slots-booking-plugin.php';
$cptslotsb_plugslug = 'wp-time-slots-booking-form';
$cptslotsb_supportlink = 'https://wptimeslot.dwbooster.com//contact-us';
$cptslotsb_supportlink_full = $cptslotsb_supportlink . '?priority-support=yes';

if (!get_option('installed_'.$cptslotsb_plugslug, ''))
    update_option('installed_'.$cptslotsb_plugslug, time() );

function cptslotsb_feedback_insert_adminScripts($hook) {
    if( 'plugins.php' == $hook  )            
    {
        wp_enqueue_style('wp-jquery-ui-dialog');
        wp_enqueue_script('jquery');
        wp_enqueue_script('jquery-ui-dialog');       
        add_action( 'admin_footer', 'cptslotsb_feedback_javascript' );            
    }
}
...
function cptslotsb_feedback() {
    global $cptslotsb_plugfile, $cptslotsb_plugslug;
    $cptslotsb_plugin_data = get_plugin_data( dirname(__FILE__).'/'.$cptslotsb_plugfile );
    $cptslotsb_plugin_version = $cptslotsb_plugin_data['Version'];
    $time = time() - get_option('installed_'.$cptslotsb_plugslug, '');
    $data = array(
                 'answer' => (@$_POST["answer"]),
                 'otherplugin' => (@$_POST["opinfo"]),
                 'otherinfo' => (@$_POST["oinfo"]),
                 'plugin' => ($cptslotsb_plugin_data['Name']),
                 'pluginv' => ($cptslotsb_plugin_version),
...
function cptslotsb_feedback_javascript() { 
    global $cptslotsb_plugslug, $cptslotsb_supportlink, $cptslotsb_supportlink_full; 
      ?>
	<script type="text/javascript">

$ = jQuery.noConflict()
$( window ).load(function() {
        document.querySelector('[data-slug="<?php echo $cptslotsb_plugslug; ?>"] .deactivate a').addEventListener('click', function(event){
            event.preventDefault()
            var urlRedirect = document.querySelector('[data-slug="<?php echo $cptslotsb_plugslug; ?>"] .deactivate a').getAttribute('href');
            ...
            $('<div title="QUICK FEEDBACK"><div style="padding:10px;">'+
               ...
               '<div class="abcreasonblock"><input type="radio" name="abcm_reason" onclick="cptslotsb_update_reason(this);" value="no-work"> The plugin didn\'t work<br />'+
                '<div id="abcm_nowork" style="margin-left:25px;display:none;padding:10px;border:1px dotted gray;color:#660000"><strong>We can help!</strong> We offer <strong>free support</strong> for this plugin. Feel free to open a support ticket at <a href="<?php echo $cptslotsb_supportlink_full; ?>"><strong><?php echo $cptslotsb_supportlink; ?></strong></a> and we will be happy to help.</div></div>'+
                
               '<div class="abcreasonblock"><input type="radio" name="abcm_reason" onclick="cptslotsb_update_reason(this);" value="-"> I don\'t like to share my information with you<br /></div>'+
               
               '<div class="abcreasonblock"><input type="radio" name="abcm_reason" onclick="cptslotsb_update_reason(this);" value="temporary-deactivation"> It\'s a temporary deactivation. I\'m just debugging an issue.<br /></div>'+
               
               '<div class="abcreasonblock"><input type="radio" name="abcm_reason" onclick="cptslotsb_update_reason(this);" value="better-plugin"> I found a better plugin<br />'+
               ...
               '<div class="abcreasonblock"><input type="radio" name="abcm_reason" onclick="cptslotsb_update_reason(this);" value="other"> Other<br />'+
      		   ...
      		   '</div></div>'
                  ...
                  buttons: [
                      ...
                      },
                      {
                        ...
                        click: function() {
                                   window.open('<?php echo $cptslotsb_supportlink_full; ?>');
      						       $(this).dialog("close");
      				           }                            
                      },
                      {
...
function cptslotsb_update_reason(field)
{
