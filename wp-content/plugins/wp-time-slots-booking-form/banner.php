	...
	function codepeople_add_promote_banner($wp_admin_bar)
	{
		...
		foreach($codepeople_promote_banner_plugins as $plugin_slug => $plugin_data )
		{
			...
			if($minimum_days <= abs($now-$value) && 0<$value)
			{
				?>
				<style>
					#codepeople-review-banner{width:calc( 100% - 20px );width:-webkit-calc( 100% - 20px );width:-moz-calc( 100% - 20px );width:-o-calc( 100% - 20px );border:10px solid #1582AB;background:#FFF;display:table;margin-top:10px;}
				...
				<div id="codepeople-review-banner">
					...
					<div class="codepeople-review-banner-content">
						<div class="codepeople-review-banner-text">
							<p>Hey, I noticed you are using the "<?php print $plugin_data[ 'plugin_name' ]; ?>" plugin – <strong>that’s awesome!</strong> Could you please do me a <strong>BIG</strong> favor and <a href="<?php echo $codepeople_promote_banner_plugins["plugin_url"]; ?>" style="color:#1582AB;font-weight:bold;text-decoration: underline;">give it a 5-star rating on WordPress</a>? Just to help us spread the word and boost our motivation. <strong>Thank you!</strong></p>                            
						</div>
						<div class="codepeople-review-banner-buttons">
                            ...
                            <br />
                            <p>Need help using the plugin? Feel free to <a href="https://wordpress.org/support/plugin/wp-time-slots-booking-form/">open a support ticket</a> and I will be happy to help you!</p> 
						</div>
