...
if( !class_exists( 'cptslotsb_iCalExport' ) )
{
    class cptslotsb_iCalExport extends cptslotsb_BaseAddon
    {
			...
			)
			{
				...
				$wpdb->insert(
								...
								array(
                                    ...
                                    'default_duration'	 => $_REQUEST["ical_default_duration"],
				...
				<div class="inside">
                   ...
                   <table class="form-table">
                    ...
                    <td><select name="cal_time_zone_modify">
                          ...
                          <?php for ($i=-23;$i<24; $i++) { ?>        
                           <option value="<?php $text = " ".($i<=0?"":"+").$i." hours"; echo $text; ?>" <?php if ($row["cal_time_zone_modify"] == $text) echo ' selected'; ?>><?php echo $text; ?></option>
                          <?php } ?>
                         </select>
                    ...
                    <td><textarea name="base_summary"><?php echo $row["base_summary"]; ?></textarea>
                         <br />
                    ...
                    <td><textarea name="base_description"><?php echo $row["base_description"]; ?></textarea>
                         <br />
                         <em>* Note: You can get the field IDs/tags from the form builder.</em>
                    </td>
                    </tr>  
                    ...
                    <td><input type="number" name="ical_default_duration" value="<?php echo $row["default_duration"]; ?>" />
                    </td>
                    </tr>                     
                  </table>
				</div>
                ...
                if ($item->ipaddr != 'remote-ical-file')
                {
                    ...
                    foreach($data["apps"] as $app)
                        if ($app["date"] >= $from && $app["date"] <= $to)
                        {
                            ...
                            echo "SUMMARY:Booking for ".$base_summary."\n";
                            echo "TRANSP:OPAQUE\n";
                            echo "END:VEVENT\n";
                        }
                }  
        ...
        function attach_ical_file( $attachments, $params, $form_id) 
        {
            ...
            $filename1 = sanitize_file_name('Appointment'.'_'.intval($params["itemnumber"]).'_admin.ics');
