	...
	class WS_Form_Form extends WS_Form_Core {
		...
		public function form_parse($form_object, $published = false) {
			...
			foreach($fields as $field) {
				...
				if(isset($form_object->groups[$group_key]->sections[$section_key]->fields[$field_key]->label)) {
					...
					$form_object->groups[$group_key]->sections[$section_key]->fields[$field_key]->label = WS_Form_Common::parse_variables_process($field->label, $form_object, false, $content_type = 'text/html', 'form_parse');
				}
				...
				if(isset($field->meta)) {
					...
					foreach((array) $field->meta as $meta_key => $meta_value) {
						...
						if(
							...
							$form_object->groups[$group_key]->sections[$section_key]->fields[$field_key]->meta->{$meta_key} = WS_Form_Common::parse_variables_process($meta_value, $form_object, false, $content_type = 'text/html', 'form_parse');
						}
					}
				}
		...
		public function data_source_process(&$form_object, &$meta_keys, $field, $field_config, $published = false, $mode = 'form_parse') {
			...
			foreach($data_source_meta_keys as $data_source_meta_key) {
				...
				$data_source->{$data_source_meta_key} = WS_Form_Common::get_object_meta_value($field, $data_source_meta_key, false);
			...
			if(
				...
				$get_return = $data_source->get($field->id, 1, $meta_key, $meta_value, true);	// true = form_parse to ignore paging
		...
		public function db_read_recent($limit = 10) {
			...
			return self::db_read_all('', '', 'date_updated DESC', $limit, '', false);
		}
